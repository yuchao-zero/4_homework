...
public class ObjectPairDriver {
  ...
  public static String largestStadium(ObjectPair[] stadiums) {
    ObjectPair objectPairs = stadiums[0];
      for (ObjectPair stadium : stadiums) {
          if (Integer.parseInt(objectPairs.getSecond().toString().replace(",", ""))
                  < Integer.parseInt(stadium.getSecond().toString().replace(",", ""))) {
              objectPairs = stadium;
          }
      }
    return objectPairs.getFirst().toString();
  }
